...
(function (root, factory) {
	...
	function createEternityInstance(configuration) {
		...
		function markOldValueAsUnstable(dbImage, event) {
            ...
            eternityGc.addUnstableOrigin(oldValueDbImage);
					}
				}
			}
		}
		...
		function setupDatabase() {
			...
			imageCausality.pulse(function() {					
				...
				if (mockMongoDB.getRecordsCount() === 0) {
					...
					// throw new Error("foo");
					eternityGc.initializeGcState(createImagePlaceholderFromDbId(collectionDbId));
				} else {
    ...
    // });
      ...
      tryForgetObject(dbImage);
		...
		// Additions
		let instance = {};
    let eternityGc = require('./eternityGC.js').setEternity(instance);
      ...
      collectAll : eternityGc.collectAll,
			oneStepCollection : eternityGc.oneStepCollection,
			log : log,
			logGroup : logGroup,
