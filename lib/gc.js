...
export function createListType(name) {
  return {
...
export function initializeList(head, listType) {
  head[listType.first] = null;
  head[listType.last] = null;
  head[listType.counter] = 0;
}

export function addLastToList(head, listType, listElement) {
...
export function addFirstToList(head, listType, listElement) {
...
export function getLastOfList(head, listType) {
  return head[listType.last];
}

export function getFirstOfList(head, listType) {
  return head[listType.first];
}

export function removeLastFromList(head, listType) {
  let lastElement = head[listType.last];
  removeFromList(head, listType, lastElement);
  return lastElement;
}

export function removeFirstFromList(head, listType) {
  let firstElement = head[listType.first];
  removeFromList(head, listType, firstElement);
  return firstElement;
}

export function removeFromList(head, listType, listElement) {
  // log("removeFromList");
  // log(listType);
...
// TODO: Remove this... 
export function deallocateInDatabase(dbImage) {
  // trace.deallocate && 
...
export function initializeGcState(gcState) {     
...
export function addUnstableOrigin(pendingUnstableImage) {
...
export function isUnstable(dbImage) {
  return typeof(dbImage._eternityParent) === 'undefined';
}

export function unstableOrBeeingForgetedInGcProcess(dbImage) {
  let result = false;
...
export function removeFromAllGcLists(dbImage) {
...
export function tryReconnectFromIncomingContents(contents) {
...
export function oneStepCollection() {
