...
export function setupGC(world) {
  ...
  class PersistentList {
    ...
    firstIsInTransaction() {
      const firstImage = this.head[this.names.first];
      if (firstImage === null) throw new Error("Check if list is empty first!"); 
      const firstObject = first[meta].object; 
      return typeof(firstObject[meta].inTransaction) === "number" && firstObject[meta].inTransaction > 0;
    }
  ...
  // Hold on to parents. Dont let them be deallocated before their children has been written to database.
  let persistentParentOfNewPersistentRootList = new PersistentList("PersistentParentOfNewPersistentRootList");
  ...
  function removeFromAllGcLists(image) {
    persistentParentOfNewPersistentRootList.remove(image);
  ...
  async function oneStepCollection() {
    ...
    if (!justReattatchedList.isEmpty()) {
      if (justReattatchedList.firstIsInTransaction()) return false; // Locked! process transactions to resolve!
    ...
    if (!justDetatchedList.isEmpty()) {
      if (justDetatchedList.firstIsInTransaction()) return false; // Locked! process transactions to resolve!
    ...
    if (!toBeReattachedList.isEmpty()) {
      if (toBeReattachedList.firstIsInTransaction()) return false; // Locked! process transactions to resolve!
    ...
    if (!unableToReattachList.isEmpty()) {
      if (unableToReattachList.firstIsInTransaction()) return false; // Locked! process transactions to resolve!
    ...
    if (!justDetatchedByApplicationList.isEmpty()) {
      if (justDetatchedByApplicationList.firstIsInTransaction()) return false; // Locked! process transactions to resolve!
  ...
  return {
    ...
    oneStepCollection,
    collectAll,
    persistentParentOfNewPersistentRootList,
